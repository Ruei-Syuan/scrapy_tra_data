...
class TravelImagesPipeline(ImagesPipeline):
    ...
    def item_completed(self, results, item, info):
        ...
        if image_paths:
            # 更改文檔的名字為直播間名+房間號
            os.rename(self.IMAGES_STORE + "/" + image_paths[0], self.IMAGES_STORE + "/full/" + item["tra_city"] + ".jpg")
            # 將圖片儲存的路徑保存到item中，返回item
            item["city_img_local"] = self.IMAGES_STORE + "/full/" + item["tra_city"] + ".jpg"#將提取到的path寫入到Item中　
